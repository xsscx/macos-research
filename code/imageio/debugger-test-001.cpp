 ...
 *
 *  @section TODO
 *  - Fix Use After Free in catch_mach_exception_raise_state_identity()
 */
...
kern_return_t catch_mach_exception_raise_state_identity(
  ...
  *new_state_cnt = old_state_cnt;
  ...
  auto mach_exception = std::make_unique<Debugger::MachException>(
                        exception_port,
                        thread_port,
                        task_port,
                        exception_type,
  ...
  {
    std::lock_guard<std::mutex> lock(Debugger::map_mutex);
    auto it = Debugger::task_to_debugger_map.find(task_port);
    if (it == Debugger::task_to_debugger_map.end() || it->second == nullptr) {
  ...
  if (!dbg->killing_target) {
    dbg->HandleExceptionInternal(mach_exception.get());
  } else {
  ...
  // No need to explicitly delete mach_exception; it's managed by std::unique_ptr and will be automatically deleted here
